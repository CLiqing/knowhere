...
class RaBitQApproxDistanceComputer final : public diskann::ApproxDistanceComputer {
 public:
    RaBitQApproxDistanceComputer(const faiss::RandomRotationMatrix* rotation, const faiss::IndexRaBitQ* rabitq,
                                 bool probabilistic_refinement)
        : rotation_(rotation),
          rabitq_(rabitq),
          probabilistic_refinement_(probabilistic_refinement),
          distance_computer_(rabitq->get_quantized_distance_computer(0, false)),
    ...
    compute_distances(const unsigned* ids, _u64 n_ids, float* distances, float threshold, bool threshold_valid,
        ...
        for (_u64 i = 0; i < n_ids; ++i) {
            ...
            const size_t code_body_size = rabitq_->rabitq.dense_layout
                                              ? (static_cast<size_t>(rabitq_->d) * rabitq_->rabitq.nb_bits + 7) / 8
                                              : (static_cast<size_t>(rabitq_->d) + 7) / 8;
...
RaBitQStore::BuildFromFloatBin(const std::string& data_path, const std::string& sidecar_path, uint8_t rbq_bits) {
    if (rbq_bits < 1 || rbq_bits > 8) {
        throw std::invalid_argument("RaBitQ database bits must be in [1, 8]");
    }
...
RaBitQStore::Validate() {
    ...
    if (rabitq_->ntotal < 0 || rabitq_->rabitq.nb_bits < 1 || rabitq_->rabitq.nb_bits > 8 ||
        rabitq_->rabitq.dense_layout != (rabitq_->rabitq.nb_bits > 1)) {
...
RaBitQStore::CreateDistanceComputer(bool probabilistic_refinement) const {
    return std::make_unique<RaBitQApproxDistanceComputer>(rotation_, rabitq_, probabilistic_refinement);
}
