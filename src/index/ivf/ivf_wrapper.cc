...
IndexIvfFactory::create_for_sq(faiss::cppcontrib::knowhere::IndexFlat* qzr_raw_ptr, const faiss::idx_t d,
    ...
    if (quantizer_type_tolower == "sq4") {
        ...
        auto quantizer_type_cnd = get_sq_quantizer_type(ivf_sq_cfg.sq_type.value());
        if (!quantizer_type_cnd.has_value()) {
            return expected<std::unique_ptr<IndexIVFSQWrapper>>::Err(Status::invalid_args,
                                                                     "Invalid scalar quantizer type");
        }
        quantizer_type = quantizer_type_cnd.value();
    }
