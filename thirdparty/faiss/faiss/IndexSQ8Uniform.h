...
// 8-bit Uniform Scalar Quantization indices with COSINE and IP support
...
//////////////////////////////////////////////////////////////////////////////////
// Distance Computers for SQ8Uniform
//////////////////////////////////////////////////////////////////////////////////

/**
 * Distance computer for SQ8Uniform with COSINE metric.
 * Normalizes query vector internally.
 * Converts L2^2 distance to cosine similarity: cosine = 1 - 0.5 * L2^2
 */
struct SQ8UniformCosineDistanceComputer : DistanceComputer {
    /// owned by this
    ...
    /// storage for normalized query
    std::vector<float> query_storage;

    SQ8UniformCosineDistanceComputer(
            const int d_,
...
/**
 * Distance computer for SQ8Uniform with IP metric.
 * Stores L2 norms squared of base vectors to convert L2^2 to IP.
 * Formula: IP(q,b) = 0.5 * (||q||^2 + ||b||^2 - L2^2(q,b))
 */
struct WithSQ8UniformNormIPDistanceComputer : DistanceComputer {
    /// owned by this
    std::unique_ptr<DistanceComputer> basedis;
    ...
    WithSQ8UniformNormIPDistanceComputer(
...
/**
 * Scalar Quantizer specialized for 8-bit uniform quantization with COSINE
 * metric. Normalizes vectors internally during train() and add() to avoid
 * modifying caller's data. Implements HasInverseL2Norms.
 */
struct IndexScalarQuantizer8bitUniformCosine : IndexScalarQuantizer,
                                               HasInverseL2Norms {
    /// Storage for inverse L2 norms (needed for refine to work correctly)
    mutable std::vector<float> inverse_l2_norms;

    IndexScalarQuantizer8bitUniformCosine(int d);
    IndexScalarQuantizer8bitUniformCosine();
...
/**
 * Scalar Quantizer specialized for 8-bit uniform quantization with IP metric.
 * Stores L2 norms squared of vectors to convert L2^2 distances to IP.
 */
struct IndexScalarQuantizer8bitUniformIP : IndexScalarQuantizer {
    /// Storage for L2 norms squared (||x||^2)
    std::vector<float> l2_norms_sqr;

    IndexScalarQuantizer8bitUniformIP(int d);
    IndexScalarQuantizer8bitUniformIP();
...
struct IndexHNSWSQ8UniformCosine : IndexHNSW {
    IndexHNSWSQ8UniformCosine();
...
struct IndexHNSWSQ8UniformIP : IndexHNSW {
    IndexHNSWSQ8UniformIP();

    IndexHNSWSQ8UniformIP(int d, ScalarQuantizer::QuantizerType qtype, int M);
};
