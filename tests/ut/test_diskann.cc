...
TEST_CASE("Test DISKANN_RABITQ bound refinement", "[diskann][rabitq]") {
    ...
    auto distance_computer = store.CreateDistanceComputer(0, true);
    ...
    auto full_distance_computer = store.CreateDistanceComputer(0, false);
...
TEST_CASE("Test DISKANN_RABITQ rejects inconsistent sidecars", "[diskann][rabitq][sidecar]") {
    ...
    const auto corrupt_and_check = [&](const std::string& suffix, const auto& corrupt) {
        std::unique_ptr<faiss::Index> index(faiss::read_index(sidecar_path.c_str()));
        ...
        faiss::write_index(index.get(), corrupted_path.c_str());
